...
export default function LocationPage(location_id) {
  ...
  async function applyAsLocationManager(user, locationId) {
    ...
    const { error } = await supabase
      ...
      .insert([
        { user_id: user.id, location_id: locationId, status: "pending" },
      ]);
  ...
  return (
    <div className="p-6">
      ...
      {user?.role === "manager" && (
        <Button
          onClick={() => {
            applyAsLocationManager(user, location_id);
          }}
        >
          Apply to join location as manager
        </Button>
      )}
    </div>
  );
